...
class GaussianDiffusion:
    ...
    def ddim_sample_loop_progressive(
        ...
        """
        middle_sample.clear()
        print(f"ddim_sample_loop_progressive device: {device}")
        if device is None:
            device = next(model.parameters()).device
        assert isinstance(shape, (tuple, list))
        print(f"ddim_sample_loop_progressive noise: {noise}")
        if noise is not None:
            img = noise
        else:
            img = th.randn(*shape, device=device)
        ...
        print(f"ddim_sample_loop_progressive progress: {progress}")
        if progress:
            # Lazy import so that we don't depend on tqdm.
            from tqdm.auto import tqdm
        ...
        for i in indices:
            ...
            with th.no_grad():
                ...
                middle_sample.append(copy.deepcopy(out["sample"]))
                # print(f"middle_sample: {len(middle_sample)} \n{middle_sample[-1]}")
                yield out
                img = out["sample"]
                # print(img)
