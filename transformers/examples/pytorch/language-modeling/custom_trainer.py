...
class Classifier_TONE(BertPreTrainedModel):
    _keys_to_ignore_on_load_missing = [r"attn.masked_bias", r"attn.bias", r"lm_head.weight"]
    ...
    def __init__(self, config, diffusion=None):
        ...
        print(diffusion)
        self.diffusion = diffusion
        print("config.train_diff_steps:", config.train_diff_steps)
        if diffusion is not None:
            self.train_diff_steps = config.train_diff_steps
        else:
            self.train_diff_steps = config.train_diff_steps
        ...
        # Initialize weights and apply final processing
        self.post_init()
        self.lm_head2 = nn.Linear(config.hidden_size, config.tone_vocab_size, bias=False)
    ...
    def forward(
        ...
        if tone_ids is not None:
            ...
            #       shift_logits.view(-1, shift_logits.size(-1)).shape)    # torch.Size([4096, 7])
            # print("shift_labels.view(-1):", shift_labels.view(-1).shape)  # torch.Size([4096])
            # assert 0
            loss = loss_fct(shift_logits.view(-1, shift_logits.size(-1)), shift_labels.view(-1))
...
class Classifier_VOWEL(BertPreTrainedModel):
    _keys_to_ignore_on_load_missing = [r"attn.masked_bias", r"attn.bias", r"lm_head.weight"]
    ...
    def __init__(self, config, diffusion=None):
        ...
        # Initialize weights and apply final processing
        self.post_init()
        self.lm_head2 = nn.Linear(config.hidden_size, config.vowel_vocab_size, bias=False)
...
class Classifier_RHYME(BertPreTrainedModel):
    _keys_to_ignore_on_load_missing = [r"attn.masked_bias", r"attn.bias", r"lm_head.weight"]
    ...
    def __init__(self, config, diffusion=None):
        ...
        # Initialize weights and apply final processing
        self.post_init()
        self.lm_head2 = nn.Linear(config.hidden_size, config.rhyme_vocab_size, bias=False)
...
class Classifier_LINE(BertPreTrainedModel):
    def __init__(self, config, diffusion=None):
        super().__init__(config)
        self.diffusion = diffusion
        self.transformer = BertModel(config)
        ...
        self.time_embeddings = nn.Embedding(self.train_diff_steps + 1, config.hidden_size)
        self.post_init()
        self.lm_head2 = nn.Linear(config.hidden_size, config.line_vocab_size, bias=False)
    ...
    def forward(
        ...
        # print(transformer_outputs[0].shape, hidden_states.shape)
