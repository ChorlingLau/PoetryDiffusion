...
def get_corpus_rocstory(data_args):
    ...
    elif data_args.experiment.startswith('ci-'):
    ...
    elif data_args.experiment.startswith('sonnet-'):
        print('loading dataset from sonnet dataset')
        sentence_lst = []
        nlp = English()
        tokenizer = nlp.tokenizer
        path = f'/home/LAB/liucm/Diffusion-LM/datasets/sonnet3355/sonnet_train.txt'
        with open(path, 'r', encoding='utf-8') as ff:
            for row in ff:
...
def main():
    ...
    global tone_vocab, vowel_vocab, line_map, rhyme_vocab
    ...
    ###################### LOAD DATASETS & dictionary #########################
            ...
            model_args.experiment.startswith('ci-') or \
            model_args.experiment.startswith('sonnet-'):
        ...
        elif model_args.experiment.startswith('ci-'):
            if 'tone' in model_args.experiment:
                tone_vocab = {'START': 0, 'END': 1, 'UNK': 2, 'PAD': 3, 'NONE': 4, 'PING': 5, 'ZE': 6}
            elif 'vowel' in model_args.experiment:
                vowel_vocab = {'START': 0, 'END': 1, 'UNK': 2, 'PAD': 3}
                vowel_lst = [str(x) for x in range(18)]
                for x in vowel_lst:
                    vowel_vocab[x] = len(vowel_vocab)
        elif model_args.experiment.startswith('sonnet-'):
            if 'line' in model_args.experiment:
                line_map = {str(i+1): i for i in range(14)}
            elif 'rhyme' in model_args.experiment:
                rhyme_vocab = {'START': 0, 'END': 1, 'UNK': 2, 'PAD': 3}
                rhyme_lst = [str(x) for x in range(6219)]
                for x in rhyme_lst:
    ...
    if model_args.model_name_or_path:
        ...
        ############# LOAD MODELS for controllable classifier ##############
                                       ...
                                       'ci-tone', 'ci-vowel', 'sonnet-line', 'sonnet-rhyme']:
            import torch
            config.vocab_size = len(tokenizer)
            print('\n Initializing the model from scratch \n' + '*' * 100)
            ...
            training_args2['diffusion_steps'] = model_args.diffusion_steps  # 500  # DEBUG
            ...
            if model_args.experiment == 'e2e-back_t2':
                ...
                if 'tone' in model_args.experiment:
                    config.tone_vocab_size = len(tone_vocab)
                    model = Classifier_TONE(config=config, diffusion=diffusion, )
                elif 'vowel' in model_args.experiment:
                    config.vowel_vocab_size = len(vowel_vocab)
                    model = Classifier_VOWEL(config=config, diffusion=diffusion, )
            elif model_args.experiment.startswith('sonnet-'):
                if 'line' in model_args.experiment:
                    config.line_vocab_size = len(line_map)
                    model = Classifier_LINE(config=config, diffusion=diffusion, )
                elif 'rhyme' in model_args.experiment:
                    config.rhyme_vocab_size = len(rhyme_vocab)
                    model = Classifier_RHYME(config=config, diffusion=diffusion, )
            ...
            path_learned = '{}/ema_0.9999_{}.pt'.format(filename, model_args.e_step)
            if model_args.experiment in ['e2e-tgt-pos', 'e2e-tgt-tree'] and model_args.learned_emb == 'no':
            ...
            elif model_args.experiment in ['e2e-tgt-pos', 'e2e-tgt-tree'] and model_args.learned_emb == 'yes':
                ...
                model.transformer.wte.weight.requires_grad = False
            elif model_args.experiment.startswith('ci-') and model_args.learned_emb == 'no':
                if 'tone' in model_args.experiment:
                    model.transformer.embeddings.word_embeddings.load_state_dict(torch.load(path_save))
                    model.transformer.embeddings.word_embeddings.weight.requires_grad = False
                elif 'vowel' in model_args.experiment:
                    model.transformer.embeddings.word_embeddings.load_state_dict(torch.load(path_save))
                    model.transformer.embeddings.word_embeddings.weight.requires_grad = False
            elif model_args.experiment.startswith('ci-') and model_args.learned_emb == 'yes':
                if 'tone' in model_args.experiment:
                    print('loading the learned tone embeddings')
                    learned_embeddings = torch.load(path_learned)['word_embedding.weight']
                    model.transformer.embeddings.word_embeddings.weight.data = learned_embeddings.clone()
                    model.transformer.embeddings.word_embeddings.weight.requires_grad = False
                elif 'vowel' in model_args.experiment:
                    print('loading the learned vowel embeddings')
                    learned_embeddings = torch.load(path_learned)['word_embedding.weight']
                    model.transformer.embeddings.word_embeddings.weight.data = learned_embeddings.clone()
                    model.transformer.embeddings.word_embeddings.weight.requires_grad = False
            elif model_args.experiment.startswith('sonnet-') and model_args.learned_emb == 'yes':
                if 'line' in model_args.experiment:
                    print('loading the learned line embeddings')
                    learned_embeddings = torch.load(path_learned)['word_embedding.weight']
                    model.transformer.embeddings.word_embeddings.weight.data = learned_embeddings.clone()
                    model.transformer.embeddings.word_embeddings.weight.requires_grad = False
                elif 'rhyme' in model_args.experiment:
                    print('loading the learned rhyme embeddings')
                    learned_embeddings = torch.load(path_learned)['word_embedding.weight']
                    model.transformer.embeddings.word_embeddings.weight.data = learned_embeddings.clone()
                    model.transformer.embeddings.word_embeddings.weight.requires_grad = False
    ...
    if model_args.experiment.startswith('synth'):
        ...
        def tokenize_function(examples):
            ...
            with CaptureLogger(tok_logger) as cl:
                # logger.info(f'examples[\'text\']: {examples["text"]}')  # 所有的句子
                sent_lst = [" ".join(seq) for seq in examples['text']]
                sent_full = " ".join(sent_lst)
                doc = nlp(sent_full)
                # logger.info(f'e.g.doc_token_pos: {doc[0]}')     # 一个词及其对应词性(token.text, token.pos_)
                ...
                # logger.info(f'pos_lst: {pos_lst}')  # 每句话的各词词性
                if model_args.experiment == 'e2e-tgt-pos':
                    ...
                    # logger.info(f'pos_tags: {pos_tags}')  # 每句话的各词词性编号
    ...
    elif model_args.experiment in ['ci-tone', 'ci-vowel']:
        control_feature = model_args.experiment.split('-')[1]
        assert model_args.task != 'data_teacher', 'should not be data_teacher.'

        def tokenize_function(examples):
            vocab_dict = raw_datasets.vocab
            control_dict = {}
            data_path = '/home/LAB/liucm/Diffusion-LM/datasets/e2e_data/'
            full_vocab = open(data_path + 'ci_vocab.txt').read().splitlines()
            control_src_path = 'ci_' + control_feature + '.txt'
            full_control = open(data_path + control_src_path).read().splitlines()
            for i in range(len(full_vocab)):
                control_dict[full_vocab[i]] = full_control[i].split()[0]

            with CaptureLogger(tok_logger) as cl:
                control_lst = []
                for seq in examples['text']:
                    control_lst.append([control_dict[x] for x in seq])

                    input_ids = [[0] + [vocab_dict.get(x, vocab_dict['UNK']) for x in seq] + [1] for seq in
                                 examples['text']]
                    if 'tone' in model_args.experiment:
                        control_tags = [[0] + [tone_vocab[x] for x in seq] + [1] for seq in control_lst]
                    elif 'vowel' in model_args.experiment:
                        control_tags = [[0] + [vowel_vocab[x] for x in seq] + [1] for seq in control_lst]
                    # print(control_tags)
                    tags_key = control_feature + '_tags'
                    result_dict = {'input_ids': input_ids, tags_key: control_tags}
        ...
        def pad_function(group_lst):
            ...
            group_lst['input_ids'] = _collate_batch_helper(group_lst['input_ids'], vocab_dict['PAD'], max_length)
            max_src_length = 64  # min(seqlen, max_src_length)
            group_lst[f'{control_feature}_ids'], group_lst['src_mask'] = \
                _collate_batch_helper(group_lst[f'{control_feature}_tags'],
                                      vocab_dict['PAD'], max_src_length, return_mask=True)
            group_lst['labels'] = [[-100] * len(x) + y for (x, y)
                                   in zip(group_lst['input_ids'], group_lst[f'{control_feature}_ids'])]
            return group_lst
    ...
    elif model_args.experiment in ['sonnet-rhyme']:
        control_feature = model_args.experiment.split('-')[1]
        assert model_args.task != 'data_teacher', 'should not be data_teacher.'
        ...
        def tokenize_function(examples):
            vocab_dict = raw_datasets.vocab
            control_dict = {}
            data_path = '/home/LAB/liucm/Diffusion-LM/datasets/sonnet3355/'
            full_vocab = open(data_path + 'sonnet_vocab.txt').read().splitlines()
            control_src_path = 'sonnet_' + control_feature + '.txt'
            full_control = open(data_path + control_src_path).read().splitlines()
            for i in range(len(full_vocab)):
                control_dict[full_vocab[i]] = full_control[i].split()[0]
            ...
            with CaptureLogger(tok_logger) as cl:
                ...
                for seq in examples['text']:
                    ...
                    input_ids = [[0] + [vocab_dict.get(x, vocab_dict['UNK']) for x in seq] + [1] for seq in
                                 examples['text']]
                    if 'rhyme' in model_args.experiment:
                        control_tags = [[0] + [rhyme_vocab[x] for x in seq] + [1] for seq in control_lst]
                    # print(control_tags)
                    tags_key = control_feature + '_tags'
                    result_dict = {'input_ids': input_ids, tags_key: control_tags}
        ...
        def pad_function(group_lst):
            ...
            max_length = 196
            group_lst['input_ids'] = _collate_batch_helper(group_lst['input_ids'], vocab_dict['PAD'], max_length)
            max_src_length = 196  # min(seqlen, max_src_length)
            group_lst[f'{control_feature}_ids'], group_lst['src_mask'] = \
                _collate_batch_helper(group_lst[f'{control_feature}_tags'],
                                      vocab_dict['PAD'], max_src_length, return_mask=True)
            group_lst['labels'] = [[-100] * len(x) + y for (x, y)
    ...
    elif model_args.experiment in ['sonnet-line']:
        control_feature = model_args.experiment.split('-')[1]
        assert model_args.task != 'data_teacher', 'should not be data_teacher.'
        ...
        def tokenize_function(examples):
            ...
            # control_dict = {}
            data_path = '/home/LAB/liucm/Diffusion-LM/datasets/sonnet3355/'
            full_vocab = open(data_path + 'sonnet_vocab.txt', encoding='utf-8').read().splitlines()
            # control_src_path = 'sonnet_' + control_feature + '.txt'
            # full_control = open(data_path + control_src_path).read().splitlines()
            # for i in range(len(full_vocab)):
            #     control_dict[full_vocab[i]] = full_control[i].split()[0]
            ...
            with CaptureLogger(tok_logger) as cl:
                ...
                for seq in examples['text']:
                    input_ids = [[0] + [vocab_dict.get(x, vocab_dict['UNK']) for x in seq] + [1] for seq in
                                 examples['text']]
                    if 'line' in model_args.experiment:
                        control_tags = [line_map[str(seq.count("eos"))]]
                    # print(control_tags)
                    tags_key = control_feature + '_tags'
                    result_dict = {'input_ids': input_ids, tags_key: control_tags}
        ...
        def pad_function(group_lst):
            ...
            #                           vocab_dict['PAD'], max_src_length, return_mask=True)
            group_lst['labels'] = [[-100] * len(x) + [y] for (x, y)
                                   in zip(group_lst['input_ids'], group_lst[f'{control_feature}_tags'])]
            return group_lst
        ...
        logger.info(f"  model_args.experiment: {model_args.experiment}")
        assert model_args.task != 'data_teacher', 'should not be data_teacher.'
        import spacy, nltk
        ...
        logger.info(f"  parser._parser.config[\"label_vocab\"]: {parser._parser.config['label_vocab']}")
        ...
        def tokenize_function(examples):
            ...
            with CaptureLogger(tok_logger) as cl:
                ...
                if model_args.experiment == 'e2e-tgt-tree':
                    ...
                    # print("run_clm.py line 1058, result_dict:", result_dict)
                    ...
                    if model_args.task == 'finetune':
                        ...
                        for parse, seq in zip(parse_lst, examples['text']):
                            chart, spans = chart_from_tree(tree_vocab, parse, verbose=True)
                            for (a, b, c) in spans:
                                input_ids = [vocab_dict.get(x, vocab_dict['UNK']) for x in f"{a} {b} {c}".split()] + [
            ...
            # print("run_clm.py line 1079, result_dict:", result_dict)
            return result_dict
            ...
            if model_args.experiment == 'e2e-tgt-tree':
                ...
                if model_args.task == 'finetune':
                    ...
                    group_lst['input_ids'] = _collate_batch_helper(group_lst['input_ids'], vocab_dict['PAD'],
                                                                   max_length)
                    group_lst['labels'] = group_lst['input_ids']
            # print("run_clm.py line 1100, group_lst:", group_lst)
            return group_lst
        ...
        with training_args.main_process_first(desc="grouping texts together"):
          ...
          model_args.experiment.startswith('e2e-tgt') or
          model_args.experiment.startswith('ci-') or
          model_args.experiment.startswith('sonnet-')) and model_args.task not in ['data_teacher', 'finetune']:
        def tokenize_function(examples):
            vocab_dict = raw_datasets.vocab
            with CaptureLogger(tok_logger) as cl:
                if model_args.task == 'classifier':
        ...
        with training_args.main_process_first(desc="grouping texts together"):
          ...
          model_args.experiment.startswith('e2e-tgt') or
          model_args.experiment.startswith('ci-') or
          model_args.experiment.startswith('sonnet-')) and model_args.task in ['data_teacher', 'finetune']:
    ...
    trainer_tokenizer = None if ((model_args.experiment in ['pos', 'synth', 'roc', 'simple-wiki', 'e2e-tgt',
                                                            'e2e-tgt-pos', 'ci-tone', 'ci-vowel', 'sonnet-line', 'sonnet-rhyme',
                                                            'e2e-tgt-tree', 'e2e-back', 'e2e-back_t2']
                                 or model_args.experiment in ['synth_emb', 'pos_emb', 'roc_emb',
                                                              'simple-wiki_emb', 'e2e-tgt_emb'])
    ...
    if training_args.do_train:
        ...
        logger.info(f"  save_model()...")
        trainer.save_model()  # Saves the tokenizer too for easy upload

        metrics = train_result.metrics
        logger.info(f"  train_result.metrics: {metrics}")

        max_train_samples = (
            data_args.max_train_samples if data_args.max_train_samples is not None else len(train_dataset)
        )
        metrics["train_samples"] = min(max_train_samples, len(train_dataset))
        logger.info(f"  metrics[\"train_samples\"]: {metrics['train_samples']}")
    ...
    if training_args.do_eval:
        ...
        try:
            perplexity = float("inf") if "eval_loss" not in metrics else math.exp(metrics["eval_loss"])
